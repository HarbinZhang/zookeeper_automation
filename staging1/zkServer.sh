...
echo "jmx jvm agent config"
PORTER_JMX_AGENT="-javaagent:/opt/porter-jmx/porter-jmx-javaagent.jar=SERVICE_NAME=$SERVICE_NAME;SERVICE_PLANET=$SERVICE_PLANET;SERVICE_DEPARTMENT=$SERVICE_DEPARTMENT;SERVICE_DATA_CENTER=$SERVICE_DATA_CENTER;TRANSLATOR_FULL_CLASS_NAME=$TRANSLATOR_FULL_CLASS_NAME;BROKER_SERVERS_URL=$BROKER_SERVERS_URL;TOPIC_NAME=$TOPIC_NAME;INTERVAL_IN_SEC=$INTERVAL_IN_SEC;EXCLUDE_REGEX=$EXCLUDE_REGEX;INCLUDE_REGEX=$INCLUDE_REGEX"

echo "porter jmx agent : $PORTER_JMX_AGENT."
...
start-foreground)
    ...
    "-Dzookeeper.root.logger=${ZOO_LOG4J_PROP}" \
    -cp "$CLASSPATH" $JVMFLAGS $PORTER_JMX_AGENT $ZOOMAIN "$ZOOCFG"
    ;;
print-cmd)
